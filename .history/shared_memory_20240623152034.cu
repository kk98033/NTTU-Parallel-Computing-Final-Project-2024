...
int main() {
    ...
    // 读取 Python 结果
    std::ifstream python_result_file("max_clique_result_python.txt");
    if (!python_result_file) {
        std::cerr << "Failed to open Python result file." << std::endl;
        return 1;
    }

    int python_max_clique_size;
    python_result_file >> python_max_clique_size;

    std::vector<int> python_max_clique(python_max_clique_size);
    for (int i = 0; i < python_max_clique_size; ++i) {
        python_result_file >> python_max_clique[i];
    }

    std::sort(best_clique.begin(), best_clique.end());
    std::sort(python_max_clique.begin(), python_max_clique.end());

    bool match = (best_clique.size() == python_max_clique_size);
    std::cout << "Results match: " << (match ? "Yes" : "No") << "\n";

    if (!match) {
        std::cout << "CUDA Max Clique: ";
        for (int i : best_clique) {
            std::cout << i << " ";
        }
        std::cout << "\n";

        std::cout << "Python Max Clique: ";
        for (int i : python_max_clique) {
            std::cout << i << " ";
        }
        std::cout << "\n";
    }
